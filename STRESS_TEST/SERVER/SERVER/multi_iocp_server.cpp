...
enum COMP_TYPE { OP_ACCEPT, OP_RECV, OP_SEND, OP_RANDUM_MOVE};
...
bool is_npc(int id) { return id < MAX_NPC; }
...
constexpr int USER_START = MAX_NPC+1;
...
void SESSION::do_random_move()
{
	...
	{
		if (objects[i]._state != ST_INGAME) continue;
		if (false == can_see(i, _id)) continue;
		objects[i].send_move_packet(_id);
	}
...
void process_packet(int c_id, char* packet)
{
	...
	case CS_LOGIN: {
		...
		AddObjectToSector(c_id, objects[c_id].x, objects[c_id].y);
		int S_index = GetSectorIndex(objects[c_id].y / S_HEIGHT, objects[c_id].x / S_WIDTH);
		objects[c_id]._sector_index = S_index;
	...
	case CS_MOVE: {
		...
		int S_index = GetSectorIndex(objects[c_id].y / S_HEIGHT, objects[c_id].x / S_WIDTH);
		if (objects[c_id]._sector_index != S_index) {
		...
		for (int obj_id : g_ObjectListSector[S_index]) {
			if (objects[obj_id]._state != ST_INGAME) continue;
			if (false == can_see(c_id, objects[obj_id]._id)) continue;
			if (obj_id == c_id) continue;
...
{
	for (int i = USER_START; i < USER_START+MAX_NPC; i++)
	{
		...
		for (int i = 0; i < MAX_NPC; ++i)
		{
			if (objects[i]._rm_time < system_clock::now() - 1s) {
				if (i == 1) {
					...
					cout << "NPC moved delay " << duration_cast<milliseconds>(move_delay).count() << "ms\n";
...
void do_Ai_hb()
{
	...
	while (true) {
		...
		cout << "NPC AI Time : " << duration_cast<milliseconds>(hb_time).count() << "ms\n";
...
void do_Ai_wk(HANDLE h_iocp)
{
	...
	while (true) {
		...
		for (int i = 0; i < MAX_NPC; ++i) {
			...
			over->_comp_type = OP_RANDUM_MOVE; 
...
void do_timer()
{
	EVENT ev = timer_queue.pop();
	if (ev.wakeup_time < system_clock::now())
	{
		objects[ev.id].do_random_move();
