		...
		if (ret == SOCKET_ERROR) {
			...
			if (err != WSAEWOULDBLOCK) {
				if (err == WSAECONNRESET || err == WSAECONNABORTED) {
					cout << "Client disconnected: " << _id << endl;
				}
				else {
...
int main()
{
	...
	u_long byte = 1;
	for (auto& cl : objects)
		ioctlsocket(cl._socket, FIONBIO, &byte);
	...
	while (true) {
		...
		if (c_socket != INVALID_SOCKET) {
			...
			if (client_id != -1) {
				...
				if (objects[client_id]._state == ST_FREE) {
					...
					objects[client_id].x = rand() % W_WIDTH;
					objects[client_id].y = rand() % W_HEIGHT;
		...
		for (auto& cl : objects) {
			if (cl._state != ST_FREE) {
				DWORD recv_flag = 0;
				int ret = recv(cl._socket, cl._recv_buf, BUF_SIZE, recv_flag);
				if (ret == SOCKET_ERROR) {
					int err = WSAGetLastError();
					if (err == WSAEWOULDBLOCK)
						continue;
					if (err == WSAECONNRESET || err == WSAECONNABORTED) {
						cout << "Client disconnected: " << cl._id << endl;
					}
					else {
						cout << "Recv Error: " << err << endl;
					}
					...
					int remain_data = ret + cl._prev_remain;
					char* p = cl._recv_buf;
					while (remain_data > 0) {
						int packet_size = p[0];
						if (packet_size <= remain_data) {
					...
					if (remain_data > 0) {
						memcpy(cl._recv_buf, p, remain_data);
					}
				}
			}
		}
