...
public:
    mutex _s_lock;
    S_STATE _state;
	atomic_bool _active;        // NPC의 활동 여부?
    int _id;
    SOCKET _socket;
    short   x, y;
...
struct EVENT {
	...
	{
		return wakeup_time > b.wakeup_time;
	}
};
...
array<SESSION, MAX_NPC + MAX_USER> objects;      // NPC 수 고정
...
void add_timer(int obj_id, EVENT_TYPE e_type, int delay)
{
	EVENT ev;
	ev.obj_id = obj_id;
	ev.e_type = e_type;
	ev.wakeup_time = chrono::system_clock::now() + chrono::milliseconds(delay);
	eql.lock();
...
void process_packet(int c_id, char* packet)
{
	...
	case CS_MOVE: { // NPC도 이동 패킷을 받을 수 있음
        ...
        for (auto& pl : objects) {
            ...
            if (false == can_see(c_id, pl._id)) continue;   // 플레이어 주위의 npc만 이동하게..
            if (pl._id == c_id) continue;
			new_viewlist.insert(pl._id);    // 인게임인 플레이어들만 or 플레이어 주변의 npc들만
            if (true == is_npc(pl._id) && false == pl._active) {
                bool expected = false;
                if (true == atomic_compare_exchange_strong(&pl._active, &expected, true))
...
void do_ai_hb()
{
    ...
    while (true) {
        ...
        cout << "Heart Beat Time = " << duration_cast<milliseconds>(hb_time).count() << "ms\n";
...
void do_ai_wk(HANDLE h_iocp)
{
    ...
    while (true) {
        ...
        for (int i = 0; i < MAX_NPC; ++i) {
			...
			PostQueuedCompletionStatus(h_iocp, 0, i, &over->_over);     // 과부화 발생
        }
...
int main()
{
    ...
    initialize_npc();
    //thread ai_thread{ do_ai_wk, h_iocp }; // 워커스레드에 떠넘...
    thread ai_thread{ do_timer, h_iocp };   // 타이머
